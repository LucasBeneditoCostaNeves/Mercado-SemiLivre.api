...
- **WHEN** the user requests an address that belongs to a different user
...
#### Scenario: Attempt to update another user's address
- **WHEN** the user attempts to update an address belonging to another user
- **THEN** the system returns HTTP 403 Forbidden
...
#### Scenario: Attempt to delete another user's address
...
All routes under `/users/:userId/addresses` MUST require a valid JWT token.
