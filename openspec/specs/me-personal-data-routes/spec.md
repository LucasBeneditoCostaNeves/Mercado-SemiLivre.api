...
Defines the requirements for the `/me/personal-data` routes, which allow any authenticated user to manage their own personal data without admin privileges. The `userId` is always derived from the JWT token, never from the URL.
