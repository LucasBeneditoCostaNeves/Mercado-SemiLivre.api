import { Body, Controller, Delete, Get, Param, Patch, Post, UseGuards, UseInterceptors, UsePipes, UploadedFiles } from "@nestjs/common"
...
export class ProductVariationController {
    ...
    @Post('/:id/images')
    @UseGuards(SellerProfileGuard)
    @UseInterceptors(FilesInterceptor('images', 8, productImageUploadMulterOptions))
    async addImages(
        @CurrentUser() user: AuthenticatedUser,
        @Param('id') id: string,
        @UploadedFiles() files: Express.Multer.File[],
    ) {
        return this.productVariationImageService.addImages(id, user.id, files ?? [])
    }
